...
int uTurnCounter = 0;
int tsCounter = 0;  // Counter for "Tetap Lurus"
...
bool isTsSelected = false;  // Flag for "Tetap Lurus"
...
void setup() {
  ...
  lcd.setCursor(0, 0);
  lcd.print("Option: TsA1+A2");
  lcd.setCursor(0, 1);
  lcd.print("LA1 RA2 UA3 Start=A0");
}

void loop() {
  if (!isStartPressed) {
    if (digitalRead(leftButtonPin) == HIGH && digitalRead(rightButtonPin) == LOW) {
      leftTurnCounter++;
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Left Turn: ");
